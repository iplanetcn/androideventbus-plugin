...
public class ShowUsagesAction extends AnAction implements PopupAction {
    ...
    ShowUsagesAction(Filter filter) {
        this.filter = filter;
    ...
    private static void chooseAmbiguousTargetAndPerform(@NotNull final Project project,
                                                        final Editor editor,
    ...
    private static FindUsagesOptions getDefaultOptions(@NotNull FindUsagesHandler handler) {
        FindUsagesOptions options = null;
        try {
            options = handler.getFindUsagesOptions(DataManager.getInstance().getDataContextFromFocusAsync().blockingGet(5, TimeUnit.SECONDS));
    ...
    private void showElementUsages(@NotNull final FindUsagesHandler handler, final Editor editor,
        ...
        final PingEDT pingEDT = new PingEDT(o -> popup.isDisposed(), 100, () -> {
            if (popup.isDisposed()) {
                return;
            }
        ...
        final ProgressIndicator indicator = FindUsagesManager.startProcessUsages(handler, handler.getPrimaryElements(),
                handler.getSecondaryElements(), collect, options, () -> ApplicationManager.getApplication().invokeLater(() -> {
    ...
    private void showHint(@NotNull String text,
        ...
        JComponent label = createHintComponent(text, handler, popupPosition, editor, maxUsages, options);
    ...
    private JComponent createHintComponent(@NotNull String text,
        ...
        InplaceButton button = createSettingsButton(handler, popupPosition, editor, maxUsages, ShowUsagesAction.HIDE_HINTS_ACTION);
